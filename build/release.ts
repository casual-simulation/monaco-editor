...
(() => {
	...
	otherFiles = otherFiles.concat(
		readFiles('node_modules/@casual-simulation/monaco-editor-core/min-maps/**/*', {
			base: 'node_modules/@casual-simulation/monaco-editor-core/'
		})
	);
	otherFiles = otherFiles.concat(
		readFiles('node_modules/@casual-simulation/monaco-editor-core/LICENSE', {
			base: 'node_modules/@casual-simulation/monaco-editor-core/'
		})
	);
...
function AMD_releaseOne(type: 'dev' | 'min') {
	const coreFiles = readFiles(`node_modules/@casual-simulation/monaco-editor-core/${type}/**/*`, {
		base: `node_modules/@casual-simulation/monaco-editor-core/${type}`
	});
	...
	for (const file of files) {
		...
		const extraContent = pluginFiles.map((file) => {
			return file.contents
				.toString()
				.replace(
					/define\((['"][a-z\/\-]+\/fillers\/\@casual-simulation\/monaco-editor-core['"]),\[\],/,
...
function ESM_release() {
	const coreFiles = readFiles(`node_modules/@casual-simulation/monaco-editor-core/esm/**/*`, {
		base: 'node_modules/@casual-simulation/monaco-editor-core/esm',
		// we will create our own editor.api.d.ts which also contains the plugins API
		ignore: ['node_modules/@casual-simulation/monaco-editor-core/esm/vs/editor/editor.api.d.ts']
	});
...
function ESM_releasePlugins() {
	...
	for (const file of files) {
		...
		for (let i = info.importedFiles.length - 1; i >= 0; i--) {
			...
			if (!/(^\.\/)|(^\.\.\/)/.test(importText)) {
				// non-relative import
				if (!/^\@casual-simulation\/monaco-editor-core/.test(importText)) {
					console.error(`Non-relative import for unknown module: ${importText} in ${file.path}`);
					process.exit(1);
				}

				if (importText === '@casual-simulation/monaco-editor-core') {
					importText = '@casual-simulation/monaco-editor-core/esm/vs/editor/editor.api';
				}

				const importFilePath = importText.substring(
					'@casual-simulation/monaco-editor-core/esm/'.length
				);
 ...
 */
function releaseDTS() {
	const monacodts = readFiles('node_modules/@casual-simulation/monaco-editor-core/monaco.d.ts', {
		base: 'node_modules/@casual-simulation/monaco-editor-core'
	})[0];
...
function releaseThirdPartyNotices() {
	const tpn = readFiles(
		'node_modules/@casual-simulation/monaco-editor-core/ThirdPartyNotices.txt',
		{
			base: 'node_modules/@casual-simulation/monaco-editor-core'
		}
	)[0];
